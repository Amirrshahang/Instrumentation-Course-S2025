...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if(htim->Instance == TIM1)
    {
        ...
        if(motor_angle >= 360) motor_angle -= 360.0;
...
HAL_TIM_Base_Start_IT(&htim1);
  ...
  while (1)
  {
    ...
    LCD_SetCursor(0, 0);
      ...
      snprintf(buffer, sizeof(buffer), "Speed:%lu RPM", motor_speed_rpm/2);
    LCD_Print(buffer);


      LCD_SetCursor(0, 1);
      //snprintf(buffer, sizeof(buffer), "Angle:%0.1f deg", motor_angle);
      snprintf(buffer, sizeof(buffer), "Angle:%0.1f deg", motor_angle/2);
      LCD_Print(buffer);
