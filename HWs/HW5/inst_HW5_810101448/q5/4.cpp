...
volatile uint32_t motor_speed_rpm = 0;
volatile float motor_angle = 0;
#define PULSES_PER_REV 268
char buffer[16];
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if(htim->Instance == TIM1)
    {
        ...
        motor_angle = ((float)pulse_count / PULSES_PER_REV) * 360.0 ;
                if(motor_angle >= 1440.0) motor_angle -= 1440.0;
...
HAL_TIM_Base_Start_IT(&htim1);
  ...
  while (1)
  {
    ...
    LCD_SetCursor(0, 0);
      ...
      snprintf(buffer, sizeof(buffer), "Speed:%lu RPM", motor_speed_rpm / 4);
    LCD_Print(buffer);


      LCD_SetCursor(0, 1);
      //snprintf(buffer, sizeof(buffer), "Angle:%0.1f deg", motor_angle);
      snprintf(buffer, sizeof(buffer), "Angle:%0.1f deg", motor_angle / 4);
      LCD_Print(buffer);
